...
func initialize_vesting{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    vesting_initialized.emit(
        beneficiary_address, start_timestamp, cliff_in_seconds, duration_in_seconds
    )
...
func vested_amount{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}() -> (
    ...
    if condition_elapsed_duration == 1:
        ...
        let (total_elapsed_mul, total_elapsed_mul_high) = uint256_mul(
            total_balance, Uint256(elapsed_time, 0)
        )
        let (vested, remainder) = uint256_unsigned_div_rem(
            total_elapsed_mul, Uint256(duration_in_seconds, 0)
        )
